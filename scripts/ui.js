...
const SELECTORS = {
    ...
    chartBarsContainer: document.getElementById('chart-bars'),
    chartDatesContainer: document.getElementById('chart-dates'),
    chartDataMessage: document.getElementById('chart-data-message'),
...
export const UI = {
     ...
     */
    openMobileMenu: () => {
        SELECTORS.navbarMenu.classList.add('open');
        SELECTORS.mobileMenuOverlay.classList.add('active');
     ...
     */
    closeMobileMenu: () => {
        SELECTORS.navbarMenu.classList.remove('open');
        SELECTORS.mobileMenuOverlay.classList.remove('active');
     ...
     */
    toggleMobileMenu: () => {
        if (SELECTORS.navbarMenu.classList.contains('open')) {
            UI.closeMobileMenu();
        } else {
            UI.openMobileMenu();
        }
    ...
    updateBudgetDisplay: (budgetInfo, settings) => {
        const budgetElement = document.getElementById('dashboard-budget-cap');
        ...
        budgetElement.innerHTML = `
            <div class="budget-header">
                ...
                <div class="budget-amount">${UI.formatCurrency(budgetCap, settings)}</div>
            </div>
            <div class="budget-progress-container">
                ...
                <div class="budget-progress-text">${spentPercentage.toFixed(1)}% used</div>
            </div>
            <div class="budget-details">
                <div class="budget-item">
                    <span class="budget-label">Spent:</span>
                    <span class="budget-value spent">${UI.formatCurrency(totalSpent, settings)}</span>
                </div>
                <div class="budget-item">
                    <span class="budget-label">${isOverBudget ? 'Over by:' : 'Remaining:'}</span>
                    <span class="budget-value ${isOverBudget ? 'over-budget' : 'remaining'}">${UI.formatCurrency(remaining, settings)}</span>
                </div>
            </div>
        `;
        
        if (isOverBudget) {
            budgetElement.classList.remove('budget-normal');
            budgetElement.classList.add('budget-over');
            budgetElement.setAttribute('aria-live', 'assertive');
        } else {
            budgetElement.classList.remove('budget-over');
            budgetElement.classList.add('budget-normal');
            budgetElement.setAttribute('aria-live', 'polite');
        }
    },
